...
module CoindcxBot
  module Exchanges
    module Binance
      # Multiplexed +@aggTrade+ stream; same lifecycle callbacks as +TradeWs+.
      class MultiplexedTradeWs
        DEFAULT_HOST = TradeWs::DEFAULT_HOST

        # +symbol_map+ — Binance UPPER symbol => CoinDCX pair (e.g. { 'BTCUSDT' => 'B-BTC_USDT' }).
        ...
        def initialize(symbol_map:, base_ws: DEFAULT_HOST, max_symbols_per_socket: 25, logger: nil, transport: nil)
          ...
          raise ArgumentError, 'multiplexed trade requires a non-empty symbol_map' if @symbol_to_pair.empty?
          ...
          @partitions = chunks.map do |chunk_syms|
            ...
            TradePartition.new(chunk_map, base_ws: base_ws.to_s.strip.chomp('/'), logger: @logger, transport: @transport_template)
          end
          @symbol_to_partition = {}
          ...
          @feeds = @symbol_to_pair.keys.to_h do |sym|
            [sym, TradeStreamFeed.new(partition: @symbol_to_partition[sym], symbol_upper: sym, coindcx_pair: @symbol_to_pair[sym])]
          end
        end
        ...
        class TradePartition
          def initialize(symbol_to_pair, base_ws:, logger:, transport:)
            ...
            @transport = nil
            @connected = false
            stream_path = @symbols.map { |s| "#{s.downcase}@aggTrade" }.join('/')
          ...
          def attach(feed)
            ...
            raise ArgumentError, "#{sym} not in trade partition" unless @symbols.include?(sym)
          ...
          def dispatch_raw(raw)
            ...
            return unless inner.is_a?(Hash) && inner['e'] == 'aggTrade'

            sym = stream_to_symbol_upper(payload['stream']) || inner['s'].to_s.upcase
            return if sym.empty?

            pair = @symbol_to_pair[sym]
            trade = TradeWs.trade_from_agg_payload(inner, coindcx_pair: pair, binance_symbol: sym)
            feed = @mutex.synchronize { @listeners[sym] }
            feed&.emit_trade(trade)
          rescue StandardError => e
            feed = sym && !sym.to_s.empty? ? @mutex.synchronize { @listeners[sym] } : nil
            if feed
              feed.emit_error(e)
            else
              @logger&.warn("[binance.multiplex.trade] #{e.message}")
            end
          end
